...
function ListingInfo() {
    ...
    const preferences =  Object.keys(listingInfo?.user?.preferences).filter(key => listingInfo?.user?.preferences[key])
    const highlights =  listingInfo?.roommateDetails && Object.keys(listingInfo?.roommateDetails?.highlights).filter(key => listingInfo?.roommateDetails?.highlights[key])

    const amenities =  listingInfo?.roommateDetails &&  Object.keys(listingInfo?.roommateDetails?.amenities).filter(key => listingInfo?.roommateDetails?.amenities[key])
        ...
        <div className='w-full h-full max-w-[1200px] mx-auto  px-10 py-10 rounded-xl grid md:grid-cols-[30%_70%] gap-5 gap-y-10 '>
            ...
            <div className=' h-fit border-2 flex flex-col gap-2  items-center justify-center shadow-xl rounded-xl p-10 w-fit  mx-auto '>
                ...
                <div className=' w-full   grid grid-cols-2  gap-x-5 mt-5 '>
                    <a className='' href={`mailto:${listingInfo?.user?.email}`}><Button text="Chat" className="w-full max-w-xs">Chat</Button></a>
                    <a href={`tel:${listingInfo?.user?.number}`}>
                    <Button text="Call" className="w-full max-w-xs place-self-start">Call</Button>
                    </a>
            ...
            <div className=' flex flex-col gap-2  shadow-xl border-2 rounded-xl p-10 ' >
                ...
                <div className='text-left border-b-2 py-5  px-5 '>
                   ...
                   <div className='flex justify-between items-center mt-3  flex-wrap gap-x-8 gap-y-3  '>
                        ...
                        <div>
                        <h3 className='text-lg font-[500] text-gray-500'>Approx Rent</h3>
                        ...
                        <div>
                            <h3 className='text-lg font-[500] text-gray-500'>Looking For </h3>
                ...
                {preferences?.map((preference , i) => <div className='rounded-2xl   bg-gray-200  px-5 py-1 ' key={i}>
                ...
                {listingInfo?.roommateDetails && <div>

                    <div className='text-left border-b-2 py-5  px-5 '>
                <h2 className='text-2xl  font-[600]'>Highlights</h2>
                <div className='flex items-center  mt-5  flex-wrap gap-3  '>
                {highlights?.map((preference , i) => <div className='rounded-2xl   bg-gray-200  px-5 py-1 ' key={i}>
                    <p className='capitalize '>{preference}</p>
                </div>)}
                </div>
                ...
                <div className='flex items-center  mt-5  flex-wrap gap-3  '>
                {amenities?.map((preference , i) => <div className='rounded-2xl   bg-gray-200  px-5 py-1 ' key={i}>
                    <p className='capitalize '>{preference}</p>
                </div>)}
                </div>
